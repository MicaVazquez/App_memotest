...
export class MesaPage implements OnInit {
  ...
  initGame() {
    ...
    const shuffledImages = [...this.images, ...this.images].sort(
      () => 0.5 - Math.random()
    );
  ...
  async checkIfGameEnded() {
    ...
    if (allMatched) {
      ...
      const email = (await this.auth.getUser())?.email; // Obtener el email del usuario
      ...
      this.db
        .saveGameStats(estadistica)
        .then(() => {
          this.isSaving = false; // Ocultar el spinner
          Swal.fire({
            title: 'PARTIDA FINALIZADA',
            text: `Tu tiempo fue de ${this.formatTime(
